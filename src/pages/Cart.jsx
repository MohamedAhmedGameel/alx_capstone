...
const Cart = () => {
  ...
  const handleUpdate = (productId, change) => {
    ...
    if (product) {
      ...
      if (newQuantity > 0) {  // Ensure quantity stays above 0
      ...
      <div className="bg-white shadow">
        ...
        <div className="px-4 py-6 sm:px-8 sm:py-10">
            ...
            <ul className="-my-8">
              ...
              <li key={product.id} className="flex flex-col space-y-3 py-6 text-left sm:flex-row sm:space-x-5 sm:space-y-0">
                <div className="shrink-0">
                  <img className="h-24 w-24 max-w-full rounded-lg object-cover" src={product.imageSrc} alt={product.imageAlt} />
                </div>
                ...
                <div className="relative flex flex-1 flex-col justify-between">
                  <div className="sm:col-gap-5 sm:grid sm:grid-cols-2">
                    <div className="pr-8 sm:pr-5">
                      ...
                      <p className="mx-0 mt-1 mb-0 text-sm text-gray-400">{product.color}</p>
                    </div>
                  ...
                  <div className="absolute top-0 right-0 flex sm:bottom-0 sm:top-auto">
                    <button type="button" onClick={()=>{hundleRemove(product.id)}} className="flex rounded p-2 text-center text-gray-500 transition-all duration-200 ease-in-out focus:shadow hover:text-gray-900">
          ...
          <div className="mt-6 flex items-center justify-between">
            ...
            <p className="text-2xl font-semibold text-gray-900"><span className="text-xs font-normal text-gray-400">USD</span> ${cart.total + 8}</p>
          </div>
        ...
        </div> : <div className="bg-gray-100 text-2xl outline-none shadow-none border-none text-center"> your cart is empty</div>}
      </div>
    </div>
  </div>
</section>
